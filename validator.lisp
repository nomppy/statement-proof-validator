...
(defun pars-state (lsym)
  (cond
    ((atom lsym) lsym)
    ((eql (car lsym) '-) (list (neg (pars-state (cdr lsym)))
                               (pars-state (cddr lsym))))
    ((atom (car lsym)) (car lsym))
    (t (mapcar #'pars-state (car lsym)))
    ))
;;   (if (char-equal #\- (char str 0))
;;       (neg (parse-state (subseq str 1)))
;;         (if (consp s)
;;             (do ((sel s (cdr sel)))
;;                  ((null sel))
;;                 (when (eql (car sel) '-)
;;                   (setf (car sel) (neg (cadr sel)))
;;                   (setf (cdr sel) (cddr sel)))))
;;         s)))
...
(defun op-in-ln (largs op)
  (let ((l1 (ln-sym (first largs)))
    ...
    (cond
      ((when (or (null l1) (null l2)) (return-from op-in-ln nil)))
      ((when (and (consp (get l1 'state))
                  (eql op (car (get l1 'state))))
         (return-from op-in-ln (values l1 l2))))
      ((when (and (consp (get l2 'state))
                  (eql op (car (get l2 'state))))
         (return-from op-in-ln (values l2 l1))))
      (t (return-from op-in-ln nil))
      )))
...
(defun mp (claim largs)
  (multiple-value-bind (lnop lnod) (op-in-ln largs '=>)
    (cond
      ((not (and lnop lnod))
       (format nil "Invalid reference lines: ~A must exist and one must contain =>." largs))
      ((not (and
             (equal (second (get lnop 'state)) (get lnod 'state))
...
(defun mt (claim largs)
  (multiple-value-bind (lnop lnod) (op-in-ln largs '=>)
    (cond
      ...
      ((not (equal (neg (second (get lnop 'state))) claim))
...
(defun mtp (claim largs)
  (multiple-value-bind (lnop lnod) (op-in-ln largs 'v)
    (cond
      ((not (and lnop lnod))
       (format nil "Invalid reference lines: ~A must exist and one must contain v." largs))
      ((not (member (get lnod 'state) (cdr (get lnop 'state)) :key #'neg :test #'equal))
       (format nil "Incorrect usage: ~A is not the negation of either argument in ~A" lnod lnop))
      ((or (equal claim (get lnod 'state))
           (not (member claim (cdr (get lnop 'state)) :test #'equal)))
       (format nil "Incorrect usage: ~A is not a junction in ~A" claim lnop))
      (t (string "Valid")))))

(defun validate-line (line)
  (let ((l (get line 'logic))
        (claim (get line 'state)))
    (format t "~A- ~A~%" line
            (cond ((null l) (string "No justification, taken as premise"))
                  ((null (fboundp (car l))) (format nil "Unknown operation: ~A" (car l)))
                  ; ((null (val-ln (cadr l))) (format nil "Invalid reference line: ~A" (cadr l)))
                  (t (funcall (car l) claim (cdr l))))
            )))
